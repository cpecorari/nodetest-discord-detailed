...
function test(skip) {
  ...
  npm.childProcess.stdout.on('data', (data) => process.stdout.write(data.toString('utf8')));
  npm.childProcess.stderr.on('data', (data) => process.stdout.write(data.toString('utf8')));
...
function aggregate(err) {
  ...
  return new Promise((resolve, reject) => {
    Promise.all([getTestResults('surefire-reports')]).then((results) => {
...
function getCoverage(path) {
  return new Promise((resolve, reject) => {
    fs.access('./target/' + path).then(
      () => {
        fs.readFile('./target/' + path, 'UTF-8').then(
          (data) => {
            xml.promises.fromXML(data).then(
              (xml) => {
...
function getTestResults(path) {
  return new Promise((resolve, reject) => {
    fs.readdir('./target/' + path, { withFileTypes: true }).then(
      (files) => {
        var promises = [];
